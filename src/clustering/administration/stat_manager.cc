...
stat_manager_t::stat_manager_t(mailbox_manager_t *mm) :
    ...
    get_stats_mailbox(mailbox_manager,
                      std::bind(&stat_manager_t::on_stats_request, this, ph::_1))
    { }
...
void stat_manager_t::on_stats_request(const get_stats_msg_t &msg) {
    coro_t::spawn_sometime(std::bind(&stat_manager_t::perform_stats_request, this,
                                     msg.reply_address,
                                     msg.requested_stats,
                                     drainer.lock()));
}

void stat_manager_t::perform_stats_request(const return_address_t &reply_address,
                                           const std::set<std::string> &requested_stats,
